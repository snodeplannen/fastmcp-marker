"""
Test script voor Gradio API met de juiste endpoint en parameters.
"""
...
def test_gradio_api_zip() -> bool:
    ...
    for i, config in enumerate(test_configs, 1):
        ...
        try:
            ...
            if len(result) >= 4:
                # markdown_content = result[0]  # Not used in this test
                # raw_content = result[1]  # Not used in this test
                ...
                if zip_path and zip_path != "":
                    ...
                    if os.path.exists(zip_path):
                        ...
                        with zipfile.ZipFile(zip_path, 'r') as z:
                            ...
                            # Defensive check for expected_files to prevent "object has no attribute '__iter__'" error
                            expected_files = config.get("expected_files", [])
                            if not isinstance(expected_files, (list, tuple)):
                                print(f"❌ expected_files is not iterable: {type(expected_files)}")
                                test_passed = False
                                expected_files = []
...
if __name__ == "__main__":
    ...
    if success:
        print("\n🎉 API test passed! ZIP download works!")
    else:
        print("\n❌ API test failed!")
